...
void ICM20948_WE::delay(size_t ms)
{
    vTaskDelay(pdMS_TO_TICKS(ms));
}

bool ICM20948_WE::begin(int32_t freq_hz)
{
    _bus->begin(freq_hz, _address);
    ...
    while (1)
    {
        ...
        if (timeout > 10)
        {
            printf("ICM20948 initialization timeout\n");
...
void ICM20948_WE::enableLowPower(bool enLP)
...
void ICM20948_WE::switchBank(uint8_t newBank)
{
    ...
    {
        currentBank = newBank;
        _bus->write(static_cast<uint8_t>(ICM20948_REG_BANK_SEL), static_cast<uint8_t>(currentBank << 4));
    }
}
...
void ICM20948_WE::writeRegister16(uint8_t bank, uint8_t reg, int16_t val)
{
    ...
    uint8_t buffer[2] = {static_cast<uint8_t>(MSByte), LSByte};

    _bus->write(reg, buffer, 2);
}
